(function(){
  ...
  let sb;
  function init(){if(!window.supabase?.createClient)return setTimeout(init,250);sb=window.supabase.createClient(S,K);inject()}
  ...
  function inject(){
    ...
    const card=document.createElement('section');card.id='studentBookingCard';card.innerHTML=`<h2>📅 حجز موعد</h2><p class="sb-muted">اختر القسم واليوم والوقت، ثم أرسل طلبك. ستظهر حالة الطلب هنا بعد مراجعته.</p><form id="studentBookingForm" class="sb-form"><label>القسم<select name="department" required><option value="">اختر القسم</option><option>القص بالليزر</option><option>الروبوتات</option><option>الإلكترونيات</option><option>الطباعة ثلاثية الأبعاد</option><option>الخشب</option></select></label><label>التاريخ<input type="date" name="date" required></label><label>الوقت<input type="time" name="time" required></label><label class="sb-full">ملاحظات (اختياري)<textarea name="notes" maxlength="1000" placeholder="اكتب سبب الموعد أو أي ملاحظة..."></textarea></label><div class="sb-full"><button class="btn primary" id="studentBookingSubmit">إرسال طلب الحجز</button><div id="studentBookingMsg" class="sb-msg"></div></div></form><div id="studentBookingsList"></div>`;
    host.appendChild(card);card.querySelector('[name="date"]').min=localToday();card.querySelector('#studentBookingForm').addEventListener('submit',submit);loadBookings()
  }
  async function submit(e){
    e.preventDefault();
    const student=getStudent(),form=e.currentTarget,msg=document.getElementById('studentBookingMsg'),btn=document.getElementById('studentBookingSubmit');
    if(!student){msg.textContent='تعذر قراءة بيانات حساب الطالب.';msg.style.color='#b42318';return}
    const fd=new FormData(form),department=String(fd.get('department')||''),date=String(fd.get('date')||''),time=String(fd.get('time')||''),notes=String(fd.get('notes')||'');
    if(!department||!date||!time){msg.textContent='فضلاً اختر القسم والتاريخ والوقت.';msg.style.color='#b42318';return}
    if(date<localToday()){msg.textContent='لا يمكن اختيار تاريخ سابق.';msg.style.color='#b42318';return}
    const payload={p_student_name:String(student.name||'').trim(),p_student_email:String(student.email||'').trim(),p_student_phone:String(student.phone||'').trim(),p_department:department,p_appointment_date:date,p_appointment_time:time,p_notes:notes};
    ...
    if(error){
      ...
      let friendly='تعذر إرسال طلب الحجز. حاول مرة أخرى.';
      if(raw.includes('محجوز'))friendly='هذا الموعد محجوز بالفعل لهذا القسم. اختر وقتًا آخر.';
      else if(raw.includes('بيانات الطالب غير مكتملة'))friendly='بيانات حساب الطالب غير مكتملة.';
      else if(raw.includes('رقم جوال الطالب غير موجود'))friendly='رقم جوال الطالب غير موجود في الحساب.';
      else if(raw.includes('القسم غير صحيح'))friendly='القسم المختار غير صحيح.';
      else if(raw.includes('تاريخ سابق'))friendly='لا يمكن اختيار تاريخ سابق.';
      msg.textContent=friendly;msg.title=raw;msg.style.color='#b42318';btn.disabled=false;return
    }
    const a=Array.isArray(data)?data[0]:data;
    try{await fetch(`${S}/functions/v1/send-appointment-email`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({student_name:a?.student_name||student.name,student_email:a?.student_email||student.email,student_phone:a?.student_phone||student.phone,department:a?.department||department,appointment_date:a?.appointment_date||date,appointment_time:String(a?.appointment_time||time).slice(0,5),notes:a?.notes||notes})})}catch(_){ }
