(function(){
  ...
  function sendMessage(text){
    ...
    fetch(FUNCTION_URL,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:text,history:history.slice(-8)})})
      ...
      .then(function(result){
        typing.remove();
        if(!result.ok||!result.data.answer)throw new Error(result.data&&result.data.error||'request failed');
  ...
  function init(){
    ...
    launch.className='ai-chat-launch';launch.type='button';launch.innerHTML='<span>🤖</span><span>اسأل فاب لاب</span>';
    var panel=document.createElement('section');
    panel.className='ai-chat-panel';panel.setAttribute('aria-label','المساعد الذكي لفاب لاب');
    ...
    var send=panel.querySelector('.ai-chat-send');
    var busy=false;
