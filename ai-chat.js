  ...
  function sendMessage(text){
    ...
    fetch(FUNCTION_URL,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:text,history:history.slice(-8)})})
      ...
      .then(function(result){
        ...
        addMessage(result.data.answer,'bot');history.push({role:'user',text:text},{role:'model',text:result.data.answer});history=history.slice(-8);
      })
      .catch(function(err){typing.remove();console.error('Fablab AI chat error:',err);addMessage(err.message==='Failed to fetch'?'تعذر الاتصال بالمساعد حاليًا. حاول مرة أخرى.':err.message,'bot');})
      .finally(function(){busy=false;send.disabled=false;input.disabled=false;input.focus();});
  }

  function init(){
    if(document.querySelector('.ai-chat-launch'))return;addStyles();
