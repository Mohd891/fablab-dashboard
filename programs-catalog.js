(function(){
  ...
  async function load(){
    try{
      ...
      if(grid){
        ...
        const more=document.createElement('div');more.className='program-more-wrap';more.innerHTML='<a class="btn primary large program-more" href="programs.html">اكتشف المزيد من البرامج ←</a>';grid.parentNode.insertBefore(more,grid.nextSibling);
        const oldPromo=document.querySelector('.game-promo'); if(oldPromo && oldPromo.parentNode!==grid.parentNode) oldPromo.remove();
      }
  ...
  function card(p,type){
    ...
    return '<article class="'+(type==='home'?'program-card':'real-card')+'">'+
      (p.image?'<img class="'+(type==='home'?'program-image':'')+'" src="'+esc(p.image)+'" alt="'+esc(p.name)+'">':'')+
      '<div class="'+(type==='home'?'program-content':'real-body')+'"><span class="tag">برنامج</span><h3>'+esc(p.name)+'</h3><p>'+esc(p.description)+'</p>'+
      (dates?'<small class="program-date">'+dates+'</small>':'')+badge+'<a class="'+(type==='home'?'program-details':'details-btn')+'" href="program.html?name='+encodeURIComponent(p.name)+'">عرض تفاصيل البرنامج ←</a></div></article>';
  }
  function renderDetail(programs){
    const name=new URLSearchParams(location.search).get('name');
    const p=programs.find(x=>x.name===name); if(!p)return;
    const root=document.querySelector('.program-detail'); if(!root)return;
  ...
  const style=document.createElement('style');style.textContent='.status-available{display:inline-flex;margin-top:12px;margin-right:8px;padding:8px 13px;border-radius:999px;background:#e9f8ef;color:#16834b;font-size:11px;font-weight:800}.status-finished{display:inline-flex;margin-top:12px;margin-right:8px;padding:8px 13px;border-radius:999px;background:#f1f3f5;color:#687385;font-size:11px;font-weight:700}.available-badge{display:inline-flex;padding:8px 14px;border-radius:999px;background:#e9f8ef;color:#16834b;font-weight:800;font-size:13px;margin-bottom:12px}.program-date{display:block;margin-top:10px;color:#7a8798}.program-more-wrap{text-align:center;margin:28px 0 8px}.program-more{min-width:240px}.real-grid .real-card .real-body h3{font-size:21px;margin:5px 0 8px}.real-grid .real-card .real-body p{font-size:12px;color:#768296;line-height:1.9;margin:0}.real-grid .real-card img{object-fit:cover}';document.head.appendChild(style);
