document.addEventListener('DOMContentLoaded',function(){
  ...
  if(grid && !document.querySelector('.game-promo')){
    ...
    var programsSection=grid.closest('section') || grid.parentElement;
    if(programsSection && programsSection.parentElement){
      var target=null;
      document.querySelectorAll('h1,h2,h3,.section-title').forEach(function(h){if((h.textContent||'').includes('اكتشف برامجنا')) target=h;});
      var targetSection=target ? (target.closest('section') || target.parentElement) : programsSection;
      targetSection.parentNode.insertBefore(game,targetSection);
    }else{
      grid.parentNode.insertBefore(game,grid);
    }
  }
  /* الصفحة الرئيسية موجهة لجميع المستخدمين: المدير والموظف والطالب.
     إنشاء الحساب يخص الطالب فقط، لذلك الزر العام يكون "تسجيل الدخول". */
  function normalizeLoginButton(){
    document.querySelectorAll('a,button').forEach(function(el){
      var txt=(el.textContent||'').trim();
      if(txt==='سجل كطالب' || txt==='تسجيل الدخول / إنشاء حساب' || txt==='إنشاء حساب / تسجيل الدخول'){
        el.textContent='تسجيل الدخول';
        if(el.tagName==='A') el.setAttribute('href','login.html');
      }
    });
  }
  normalizeLoginButton();
  setTimeout(normalizeLoginButton,250);
  setTimeout(normalizeLoginButton,800);
